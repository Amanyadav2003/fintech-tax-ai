...
function Documents({ onApplyValues, reviewSeed }) {
  ...
  const load = () => { setLoading(true); setError(''); return api.get('documents').then(response => setDocuments(Array.isArray(response.data) ? response.data : [])).catch(() => { setDocuments([]); setError('Unable to load your documents.'); }).finally(() => setLoading(false)); };
  useEffect(() => { load(); }, []);
  ...
  return <section className="documents-page"><div className="documents-heading"><div><p className="eyebrow">Secure storage</p><h1>My Documents</h1><p>Upload as many files as you need. Every suggested value stays here until you review and confirm it.</p></div></div>{error && <p className="document-error" role="alert">{error} <button type="button" className="text-action" onClick={load}>Retry</button></p>}
