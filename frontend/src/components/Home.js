...
function Home({ user, onStart, onHistory, onViewResult, onOpenDocuments, onUseDocumentValues }) {
  ...
  const useValues = (values) => { setSelectedDocument(null); onUseDocumentValues(values); };
  ...
  return <section className="home-page"><div className="home-heading"><div><p className="eyebrow">TaxMate AI</p><h1>Welcome, {user.name || user.email}</h1><p>Keep your filing work organized and ready for the next step.</p></div><button className="primary-action" onClick={onStart}>Start New Analysis <ArrowRight size={18} /></button></div>
    ...
    {selectedDocument && <DocumentPanel documentType={selectedDocument} onClose={() => setSelectedDocument(null)} onUseValues={useValues} onOpenVault={() => { setSelectedDocument(null); onOpenDocuments(); }} />}</section>;
}
export default Home;
