...
function ProfileMenu({ user, onProfile, onDashboard, onHistory, onExpenses, onDocuments, onLogout, onAdminSupport, darkMode, onToggleDarkMode }) {
  ...
  return <>
    ...
    <AnimatePresence>
      {open && <><motion.div className="drawer-overlay" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setOpen(false)} /><motion.aside className="profile-drawer" initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ type: 'spring', damping: 26, stiffness: 240 }} aria-label="Account navigation">
        ...
        <nav className="drawer-nav"><button onClick={() => closeAnd(onProfile)}><UserRound size={18} />My Profile</button><button onClick={() => closeAnd(onDashboard)}><FileText size={18} />Compliance Dashboard</button><button onClick={() => closeAnd(onHistory)}><FileText size={18} />History</button><button onClick={() => closeAnd(onExpenses)}><Receipt size={18} />Expense Tracker</button><button onClick={() => closeAnd(onDocuments)}><FileText size={18} />My Documents</button><div className="drawer-section"><button type="button" className="drawer-section-title" onClick={() => setNotificationsOpen(value => !value)}><Bell size={18} />Notification Preferences</button>{notificationsOpen && <label className="drawer-toggle"><span>Email reminders</span><input type="checkbox" checked={reminders} onChange={updateReminders} /></label>}</div><div className="drawer-section"><button type="button" className="drawer-section-title" onClick={() => setHelpOpen(value => !value)}><CircleHelp size={18} />Help &amp; Support</button>{helpOpen && <SupportPanel onOpenAdminDashboard={onAdminSupport} />}</div><div className="drawer-section"><div className="drawer-section-title"><ShieldCheck size={18} />Terms &amp; Privacy</div><div className="drawer-links"><a href="/terms">Terms of Service</a><a href="/privacy">Privacy Policy</a></div></div></nav>
        <button className="theme-toggle" onClick={onToggleDarkMode}>{darkMode ? <Sun size={18} /> : <Moon size={18} />} {darkMode ? 'Light mode' : 'Dark mode'}</button>
